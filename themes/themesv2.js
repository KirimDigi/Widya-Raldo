...
var initCoreInvitation = function initCoreInvitation() {
  ...
  // Parse URL parameter 'to' from query string or hash
  ...
  if (toParam) {
    guestName = decodeURIComponent(toParam).trim();
    // Normalisasi 'Kamu & Dia' / 'Kamu dan Dia' / variations -> 'Kamu & Aku'
    var norm = guestName.toLowerCase().replace(/\s+/g, ' ');
    if (norm === 'kamu & dia' || norm === 'kamu &amp; dia' || norm === 'kamu dan dia') {
      guestName = 'Kamu & Aku';
    }
  }

  // Update dataset guest so any other script using it has the correct value
  if (satuMomen) {
    satuMomen.dataset.guest = guestName;
  }
  ...
  var qrGuestName = document.getElementById("qrGuestName");
  if (guestName && qrGuestName) {
    qrGuestName.innerHTML = guestName;
  }

  var rsvpComponent = document.querySelector('rsvp-component');
  if (guestName && rsvpComponent) {
    rsvpComponent.setAttribute('name', guestName);
  }
